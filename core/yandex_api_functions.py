...
async def create_assistant():
    ...
    data = [sdk.files.upload(f"core/data/method{i}.txt") for i in range(1, 6)]
    operation = sdk.search_indexes.create_deferred(
        data, index_type=TextSearchIndexType()
    )
...
async def get_ans_by_assist(exp, spec, place):
    ...
    text_index_thread.write(
        f"Дай советы по тому, как улучшить преподавание уроков. Приведи примеры (удачные и неудачные)."
...
async def create(file_path):
    headers = {
        "keyId": settings.API_KEY_ID,
        "keySecret": settings.API_KEY_SECRET,
    }
    files = {}
...
async def get_text(user_prompt, data, questions=False):
    system_prompt = (
        "Ты - профессиональный спикер. Посмотри данный текст и дай рекомендации по его улучшению. "
    ...
    if questions:
        system_prompt = (
            "Представь, что ты - студент. Задай трудные для преподавателя вопросы, "
