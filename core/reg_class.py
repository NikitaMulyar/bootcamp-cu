...
class RegClass:
    ...
    async def start(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if user_exists:
            await update.message.reply_text(f'✅ {user_exists.fio}, ты уже есть в системе!',
        ...
        await update.message.reply_text('🔮 Привет-привет!\n'
                                        ...
                                        'Но сначала нужно пройти регистрацию. Пожалуйста, введит твой адрес эл. почты')
        await session.close()
    ...
    async def get_fio(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text('✅ Записал! Теперь напиши свой опыт работы')
    ...
    async def get_experience(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await update.message.reply_text('✅ Записал! Теперь напиши свою специальность')
        return self.state_speciality

    async def get_speciality(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        speciality = update.message.text
        context.user_data['register']['speciality'] = speciality
        await update.message.reply_text('✅ Записал! Теперь напиши, где и кем ты планируешь работать')
    ...
    async def get_place(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        session = await db_helper.get_session()
        user = User(**context.user_data['register'],
        ...
        await update.message.reply_text('✅ Зарегистрировал!', reply_markup=await self.menu())
        return ConversationHandler.END
